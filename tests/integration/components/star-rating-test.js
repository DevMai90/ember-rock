...
module('Integration | Component | star-rating', function(hooks) {
  ...
  test('Renders the full and empty stars correctly', async function(assert) {
    ...
    this.set('rating', 2);
    assert.dom('[data-test-rr="full-star"]').exists({ count: 2 }, 'The right amount of full stars is rendered');
  ...
  test('Calls onUpdate with the correct value', async function(assert) {
    this.set('rating', 2);
    this.set('updateRating', (rating) => assert.step(`Updated to rating: ${rating}`));
